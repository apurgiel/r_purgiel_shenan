...
rach_df <- list(
  ...
  data.frame(task = "Name one thing that you'll do\ntomorrow better than you did today.", foot = NA, pic = 'deepskyblue', other = F),
   data.frame(task = "List 10 things you're thankful for.", foot = NA, pic = 'deepskyblue', other = F),
     data.frame(task = "Keep a mood diary\n for the day.", foot = NA, pic = 'deepskyblue', other = F),
  data.frame(task = "Note 5 things in your yard that\nyou've never noticed before.", foot = NA, pic = 'deepskyblue', other = F),      
  ...
  data.frame(task = "Cross something off the to-do list\nthat you've been putting off.", foot = NA, pic = 'cyan', other = F),
  data.frame(task = "Accept help from someone at work\nwith items on your to-do list.", foot = NA, pic = 'cyan', other = F),
  ...
  data.frame(task = "Mail someone a letter for\no apparent reason.", foot = NA, pic = 'palevioletred1', other = F),
  data.frame(task = "Volunteer somewhere.\nDouble points for meals on wheels.", foot = NA, pic = 'palevioletred1', other = F),
     data.frame(task = "Spend an hour on a gift for someone.", foot = NA, pic = 'palevioletred1', other = F),    
  ...
  data.frame(task = "Cook something you've never\ncooked before.", foot = NA, pic = 'sandybrown', other = F),
  data.frame(task = "Cook one of your favorite meals.", foot = NA, pic = 'sandybrown', other = F),
      data.frame(task = "Cook a meal with your crops,\ny'old farmer lady.", foot = NA, pic = 'sandybrown', other = F),
  data.frame(task = "Cry from laughing too hard.", foot = NA, pic = 'sandybrown', other = F),
    data.frame(task = "Throw a snowball at someone.", foot = NA, pic = 'sandybrown', other = F),
  data.frame(task = "Drink some beer\non the porch.", foot = NA, pic = 'sandybrown', other = F),
  data.frame(task = "Tell an inside joke that no\none who's present will get.", foot = NA, pic = 'sandybrown', other = F),
  
     data.frame(task = "Make a conscious attempt to not be the\none to break an awkward silence.", foot = NA, pic = 'sandybrown', other = F),  
  
  data.frame(task = "Watch the sunset from start to finish\nwithout going blind.", foot = NA, pic = 'sandybrown', other = F),

  
  
  data.frame(task = "Spend some time cleaning the house.\nExtra points if done with Nick and Mitchell.", foot = NA, pic = 'plum2', other = F),  
  ...
  data.frame(task = "Go 24 hours without alcohol.\nYou can make up for it tomorrow.", foot = NA, pic = 'seagreen1', other = F), 
  ...
  data.frame(task = "Drive alone without music and\nphone for 20 minutes.", foot = NA, pic = 'lawngreen', other = F),
  data.frame(task = "Listen to a genre of music you\ntypically don't listen to.", foot = NA, pic = 'lawngreen', other = F),     
  data.frame(task = "Check out some live music.", foot = NA, pic = 'lawngreen', other = F),
  data.frame(task = "Listen to a song while doing\nabsolutely nothing else.", foot = NA, pic = 'lawngreen', other = F),
     data.frame(task = "Dance to a song when\nyou're by yourself.", foot = NA, pic = 'lawngreen', other = F),
   data.frame(task = "Dance to a song\nwith someone else.", foot = NA, pic = 'lawngreen', other = F),
   data.frame(task = "COMPLETELY BELT OUT a song\n when you're by yourself.", foot = NA, pic = 'lawngreen', other = F),  
   data.frame(task = "Sing (or try to sing) a song\nfrom start to finish.", foot = NA, pic = 'lawngreen', other = F),
   data.frame(task = "Share a song with someone who\nyou think will like it.", foot = NA, pic = 'lawngreen', other = F),
  data.frame(task = "Make a song verse,\n or a poem, I guess...", foot = NA, pic = 'lawngreen', other = F),  
     data.frame(task = "Daaance byyy da liiight\no' da moooooooooon.", foot = NA, pic = 'lawngreen', other = F),
  
  
  

  
  data.frame(task = "Call a cousin unexpectedly.", foot = NA, pic = 'sienna1', other = F),
  data.frame(task = "Start a Purgiel 15 roll call.", foot = NA, pic = 'sienna1', other = F),
    data.frame(task = "Meet up with some Tam crew peeps.", foot = NA, pic = 'sienna1', other = F),
    data.frame(task = "Make one of those weird pickle, mayo,\npeanut butter, cheese+dorrito sammiches.", foot = NA, pic = 'sienna1', other = F),
  
     data.frame(task = "Make (or attempt) a dying cow noise. Bonus\npoints if done while picking up Mitchell.", foot = NA, pic = 'sienna1', other = F),
     data.frame(task = "Do something in public to embarass Mitchell.", foot = NA, pic = 'sienna1', other = F),
       data.frame(task = "Do something in public to embarass Nick.", foot = NA, pic = 'sienna1', other = F),
  data.frame(task = "List all the Popsisms you can think of.", foot = NA, pic = 'sienna1', other = F),
      data.frame(task = "Watch a VHS (or something\nwe used to have on VHS).", foot = NA, pic = 'sienna1', other = F),   
  data.frame(task = "Play some dice. YARRRRR!", foot = NA, pic = 'sienna1', other = F),
  data.frame(task = "Complete a puzzle.", foot = NA, pic = 'sienna1', other = F),
  
  data.frame(task = "Speak French to someone for >2 minutes.\nNo, it doesn't have to make sense.", foot = NA, pic = 'sienna1', other = F),
  ...
  data.frame(task = "Check box because you attempted\nand failed a task today.", foot = NA, pic = 'yellow1', other = F),
